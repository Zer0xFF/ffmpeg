...
int ff_frame_thread_init(AVCodecContext *avctx)
{
    ...
    for (i = 0; i < thread_count; i++) {
        ...
        *copy->internal = *src->internal;
		...
		//mask = 0x2|0x4|0x8|0x10|0x20;

		//int ret = scePthreadAttrSetaffinity(&attr, mask);

		char buff[1024];
		snprintf(buff, 1024,"%s_%d","ffMpeg_",i);
		err = AVERROR(scePthreadCreate(&p->thread, &attr, frame_worker_thread, p, buff));
