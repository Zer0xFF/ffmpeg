...
#define NUM_TABLE_ENTRIES 8192
static int firstTime = 1;
static tMemoryRecord allocationTable[NUM_TABLE_ENTRIES];
static int allocationCount = 0;
static int printCount=0;


void AddToAllocationTable(void* ptr, int size)
{
	if(firstTime == 1)
	{
	...
	{
		if (allocationTable[i].ptr == 0)
		{
			allocationTable[i].ptr = ptr;
			allocationTable[i].size = size;
			break;
		}
	}
}
...
void RemoveFromAllocationTable(void* ptr)
{
	for(int i=0;i<NUM_TABLE_ENTRIES;++i)
	{
		if (allocationTable[i].ptr == ptr)
		{
			allocationTable[i].ptr = 0;
			allocationTable[i].size = 0;
		}
	}
}

int GetTotalAllocationSize()
{
	int totalSize = 0;
	for(int i=0;i<NUM_TABLE_ENTRIES;++i)
	{
		totalSize+=allocationTable[i].size;
	}
	return totalSize;
}
#endif
...
void av_free(void *ptr)
{
	...
	{
		int totalAllocated = GetTotalAllocationSize();
		printf("*--*--*--*-- Alloc Count = %d total size = %d\n",allocationCount, totalAllocated);
	}
	printCount++;
#endif
}
